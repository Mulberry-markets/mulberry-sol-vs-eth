...
use crate::consts::{ADMIN_WALLETS, GLOBAL_AUTH_SEED, USER_ACCOUNT_SEED};
...
pub fn handle_claim_win(ctx: Context<ClaimWin>) -> Result<()> {
    ...
    if ctx.accounts.signer.key().to_string() != ADMIN_WALLETS {
    ...
    let user_bet = if let Some(user_bet) = game.get_user_bet(ctx.accounts.owner.key()) {
        user_bet
    } else {
